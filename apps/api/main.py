...
function sbLoad(){
  ...
  // Always ensure at least one event page
  if(!SB_PAGES.find(p => p.type === 'event')){
    SB_PAGES.unshift({id:'event_main', type:'event', icon:SB_DEFAULT_ICON, title:'My Event'});
    sbSave();
  }
}
...
function sbInit(){
  sbLoad();
  sbRender();
  // Activate the first event page by default (or last active)
  const stored = localStorage.getItem('eventful_sb_active');
  const found = stored && SB_PAGES.find(p => p.id === stored);
  sbActivate(found ? stored : SB_PAGES[0].id, false);
...
function sbRender(renamingId){
  ...
  list.innerHTML = SB_PAGES.map((p, i) => {
    const isActive = p.id === SB_ACTIVE_ID;
    const cls = 'sb-item' + (isActive ? ' active' : '');
    const icon = p.icon || (p.type === 'event' ? SB_DEFAULT_ICON : SB_NOTE_ICON);
    if(p.id === renamingId){
      return `<div class="${cls}" data-id="${p.id}">
        <span class="si-icon">${icon}</span>
        <input class="sb-rename-input" id="sb-rename-${p.id}" value="${escapeHtml(p.title)}" placeholder="Untitled"
...
function sbActivate(id, save=true){
  ...
  const eventView = document.getElementById('app-main');
  const noteView = document.getElementById('note-view');
  const mainContent = document.querySelector('#app-main > .sticky-bar, #app-main > .container');
...
// ---------- Note editor (renders into #note-editor-wrap) ----------
...
function notesSyncTitle(){
  ...
  const titleEl = document.getElementById('notes-title');
  if(!titleEl) return;
  const page = SB_PAGES.find(p => p.id === SB_ACTIVE_ID);
  if(!page) return;
  page.title = titleEl.value;
  const item = document.querySelector(`.sb-item[data-id="${SB_ACTIVE_ID}"]`);
  if(item){ const span = item.querySelector('.si-title'); if(span) span.textContent = page.title || 'Untitled'; }
  notesDebounceSave();
}
...
function notesDebounceSave(){
  ...
  NOTES_SAVE_TIMER = setTimeout(() => {
    if(!SB_ACTIVE_ID) return;
    const page = SB_PAGES.find(p => p.id === SB_ACTIVE_ID && p.type === 'note');
    if(!page) return;
    const titleEl = document.getElementById('notes-title');
    const bodyEl = document.getElementById('notes-body');
    if(titleEl) page.title = titleEl.value;
    if(bodyEl) page.body = bodyEl.value;
    sbSave();
  }, 400);
}
